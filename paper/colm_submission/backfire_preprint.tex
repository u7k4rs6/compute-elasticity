...
token-entropy gate moves accuracy more than 0.002 from fixed-budget voting at
$N=64$. The mechanism is direct:
...
and 98.6\% (Llama). Llama has exactly 64 stored samples per problem; 50 of the
198 Qwen problems carry between 65 and 103, retained from earlier sampling
runs. $\text{MV\_acc}(N)$ draws subsets of size $\min(N, n)$ without
replacement from all $n$ stored samples for a problem, so for those 50 problems
the estimate at $N=64$ is taken over a larger pool than for the rest.
...
yet accuracy remains flat. The gate's advantage is compute
rather than accuracy: it reaches those accuracies at a mean of 40.4 samples per
problem for Qwen and 45.6 for Llama, against a flat 64 for the baseline, or
63\% and 71\% of the budget. Those capture figures are measured against
...
Agreement routing therefore buys a compute saving and contributes essentially
nothing to accuracy. The gate spends $k$ probe
...
for the weaker model higher agreement does not indicate higher accuracy. The
calibration data make this concrete.
...
gain. We report the recomputed values throughout and leave the pre-registration
as tagged.
