...
confirmatory analysis. The repository, with tagged commits, is available at
\url{https://github.com/u7k4rs6/compute-elasticity}. We report exploratory,
confirmatory, and pooled (198) values throughout, but decide PASS/FAIL on the
confirmatory set only.
...
yet accuracy remains flat. The gate spends $k$ probe
...
inference cost equal. The entropy gate is evaluated entirely within the
confirmatory 151, since logprobs exist only for that split. Measured
...
In absolute terms the entropy gate is likewise statistically indistinguishable
from fixed-budget voting. Mean per-token entropy modestly predicts which problems
...
Localized entropy (computed over final-answer tokens only) was not computable
from stored data, which retained only the mean scalar; this is a limitation of
the pipeline rather than a finding.
