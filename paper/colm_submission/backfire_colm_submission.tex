...
problems for Qwen2.5-7B and 65.7\% for Llama-3-8B, with Qwen the primary
demonstration and Llama corroborating the direction from a near-chance
...
17 for Llama, an oracle bound requiring ground truth rather than a deployable
method. No verifier-free gate reaches it: a plurality-agreement gate captures
19\% and 23\% of that headroom on the pooled set, and neither it nor a
token-entropy gate leaves accuracy statistically distinguishable from
fixed-budget voting. The mechanism is direct:
...
Qwen, and for Llama that bin is less accurate than its lowest-agreement
bin. We
...
\begin{itemize}
  ...
  \item We show the per-problem routing headroom is real: a grid oracle (best
        ...
        that headroom on the pooled set. Neither it nor a token-entropy gate
        reaches the oracle or beats fixed-budget voting meaningfully.
...
bound, and the verifier-free agreement and entropy gates. The entropy series is
computed on the confirmatory 151 only, a harder subset than the pooled 198 used
for the other series, so its lower position does not by itself indicate a worse
method.}
\label{fig:pareto}
\end{figure}
...
0.313). Against that ceiling, the agreement gate captures 18.7\%
...
yet accuracy remains flat. Fixed-budget voting at $N=64$ captures 19.2\% for
Qwen and 24.3\% for Llama by the same measure, so the gate captures less of
the headroom than the baseline it is meant to improve on. The negative result
is therefore sharper than a shortfall against the oracle: routing on agreement
is worse than not routing at all. This is consistent with PH2 in
Table~\ref{tab:confirmatory}, where the agreement gate's capture is near zero
for Qwen and negative for Llama. The gate spends $k$ probe
samples on every problem, then either returns the probe plurality (if the
plurality fraction $\geq \tau$) or proceeds to $N=64$; matched compute means
the fixed-budget baseline is evaluated at the gate's realized mean
samples-per-problem rather than a flat $N=64$, so the comparison holds total
inference cost equal.
...
bin. Llama's accuracy is not monotone in agreement, rising and then falling
across the three bins, so higher agreement does not indicate higher accuracy
for it in either direction. High self-consistency is therefore confidently
wrong about as often as
...
\textbf{Limited gate family.} We evaluate two verifier-free signals, plurality
agreement and mean token entropy. Richer signals such as final-answer log-probability
margins or confidence dynamics across samples could behave differently, and our
negative result does not rule them out. Evaluating final-answer margins requires
per-token log-probability arrays, which our pipeline did not store (it retained only a
mean entropy scalar); this is left to future work.
...
Qwen and corroborated by a second family from a near-chance baseline. The
accuracy this leaves on the table is real but unreachable with a verifier-free
agreement or entropy gate, because confidence
