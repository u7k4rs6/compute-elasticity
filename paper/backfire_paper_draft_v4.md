...
> **Superseded historical draft.** This file is retained for provenance and is
> not the current version of the paper. The authoritative source is
> `paper/colm_submission/backfire_colm_submission.tex` (with
> `backfire_preprint.tex` as its non-anonymous twin). This draft lacks the
> per-domain breakdown, the gate-family and threshold-selection limitations,
> and the expanded positioning against prior work. Where the two disagree, the
> `.tex` is correct.
